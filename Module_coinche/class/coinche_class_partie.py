...
class partie():
     ...
     def jouer_partie(self):
         ...
         if coinche_function.choisir_atout(self.manche) : #choisir valeur par defaut pour les test
             self.manche.debut(j)
             for i in range(8):
                if not self.hidden:
                    print("pli {} : \n \n".format(i))
                j=coinche_function.jouer_pli(self.manche, j, i+1) #erreur dans le decompte des plis confusion avec les tas joueur bug a iteration2 a priori fonctionne : confusion entre la position dans la main et celles des cartes possibles
             for k in range(2):
                coinche_function.affiche_cartes(self.manche.equipes[k].pli.cartes, self.manche.equipes[k].pli.name)   
             self.fin_manche()
