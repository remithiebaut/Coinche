...
from coinche_function import affiche_cartes, gain_pli, choisir_carte
import generical_function as generic
from coinche_class_main import Main
from coinche_class_carte import Carte
from coinche_class_equipe import Equipe
...
class Manche():
    ...
    def jouer_pli(self,joueurs, nombre_plis, aleatoire=True): #•fonctionne
        ...
        couleur_choisie=joueurs[0].main.jouer_carte( self.pli, choisir_carte(joueurs[0].main.cartes, joueurs[0].name,aleatoire=joueurs[0].aleatoire))    
    
        for j in joueurs[1:]:
            affiche_cartes(self.pli.cartes, self.pli.name,j.aleatoire)    
            carte_choisie=choisir_carte(self.cartes_possibles( couleur_choisie, j),j.name,aleatoire=j.aleatoire)
