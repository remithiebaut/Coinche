...
class Manche():
    ...
    def cartes_possibles(self, couleur_choisie, j):
        ...
        if couleur_choisie==self.atout : 
            ...
            #cas 1.11 : atout plus fort        
                for carte in j.main.cartes :
                    if carte.atout and carte.numero!= None and carte.valeur > self.pli.cartes[self.pli.gagnant()].valeur : #il faut checké que les cartes sont présentes 
                        atouts.append(carte)
        ...
        if self.atout in const.liste_couleur[:4]:
            ...
            if self.pli.gagnant()%2==len(self.pli.cartes)%2: #permet de se defausser sur partenaire
               return j.main.cartes
    ...
    def jouer_pli(self,joueurs, nombre_plis, aleatoire=True): #•fonctionne
        ...
        for j in joueurs[1:]:
            ...
            cartespossibles=Main("Cartes possibles")
